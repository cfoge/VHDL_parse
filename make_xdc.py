...
if not ((len(sys.argv) == 3)|(len(sys.argv) == 4)): #check for correct number of arguments
    print('This script needs two inputs, .XDC output file name and the input Top level .vhd file.')
    print('example: check_xdc.py output input.vhd (opt 3rd argument "q" generates Quartus output')
    sys.exit(1)
...
del vhd[portEnd[0]:len(vhd)] #remove from after port declaration, do this first to preserve indexing
...
for i in vhd:
    ...
    if(":" in i):
        ...
        if("downto") in i:
            ...
            portWidthSize = (int(portWidth[0]) + 1) - int(portWidth[1]) # doesn't handle inverted declarations (eg 0 down to 7)
            portnameCleaned = re.sub(r'[- ]',"", i[0:match.start()]) # remove empty spaces and --
            portName.append(    [   portnameCleaned, portWidthSize, portWidth[0], portWidth[1]]   )
        else:
            portnameCleaned = re.sub(r'[- ]',"", i[0:match.start()]) # remove empty spaces and --
...
if (quartus == True):
    for i in portName:
        if i[1] > 1: # if the port width is greater then 1 break each bit out to a separate pin
            for j in range(i[1]):
                f.writelines("set_location_assignment XXX  -to " + i[0] + "[" + str(j+int(i[3])) + "]\n")
        else:
    ...
    for i in portName:
        if i[1] > 1: # if the port width is greater then 1 break each bit out to a separate pin
            for j in range(i[1]):
                f.writelines("set_property PACKAGE_PIN XXX [get_ports {" + i[0] + "[" + str(j+int(i[3])) + "]" "}]\n")
        else:
