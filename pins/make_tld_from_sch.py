...
def rearrange_and_group_strings(strings):
    ...
    contains_clk_vcxo = [s for s in strings if s not in contains_debug and ("clk" in s or "Insert File path" in s)]
    ...
    # Step 2: Sort all remaining strings in alphabetical order ignoring everything before and including the first "Insert File path"
    contains_clk_vcxo.sort(key=lambda x: x.split('_', 1)[-1])
    remaining_strings.sort(key=lambda x: x.split('_', 1)[-1])

    # Step 3: Group strings based on the substring between the first and second "Insert File path"
    grouped_strings = {}
    for s in remaining_strings:
        key = s.split('_')[1]  # Get the substring between the first and second "Insert File path"
        grouped_strings.setdefault(key, []).append(s)

    # Combine strings containing "clk" or "Insert File path" with the grouped strings
...
for line in netlist:
    if((not("Insert File path") in line) and (chip_id.lower() in line.lower())):
    ...
    if((not("Insert File path") in line) and not(chip_id.lower() in line.lower())) and ('pl' in line.lower()) and "r" in line.lower():
        signal_name = extract_text_between_single_quotes(line)
...
if len(possible_pin_list) > 0:
    for line in netlist:
        for possible_pin in possible_pin_list:
            ...
            if((possible_pin[1].lower() in line.lower()) and (possible_pin[1] != "Insert File path") ):# and (chip_id.lower() in line.lower())):
