...
if not ((len(sys.argv) == 2)|(len(sys.argv) == 3)): #check for correct number of arguments
    print('This script needs one or two inputs, the first is a vhdl file, the second (optional) a paramiter')
    print('2nd arg "f" show all, "a" show attributes, "v" show variables, "c" show constants, "p" show processes, "s" show signals' )
...
print("Running list entity, components, ect... in " + color.YELLOW + sys.argv[1] + color.END)
...
if show_sig == True:
    ...
    for i in range(len(signal_vhdl)):
        print("        --" + signal_vhdl[i])
print(color.END + color.GREEN + "---------------------------------------------------" + color.END)
