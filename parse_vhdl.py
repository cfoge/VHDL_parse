...
class vhdl_obj(object):
    ...
    def who_is(self):
        ...
        for i in range(len(self.children_name)):
            print("   --" + str(self.children_name[i].name) + "  --> " +  str(self.children_name[i].mod))
        print("  Component")
...
def parse_vhdl(file_name):
    ...
    for i in vhd:
        ...
        if i.startswith("--"):
            continue
        elif(("library" in i)&(";" in i))or((("use" in i)or("USE" in i))&((";" in i)or("," in i))):
            ...
            if ("," in tmp2[1]):
                tmp2[1] = tmp2[1].split(",") 
                tmp2[1] = tmp2[1][0]
            entity_vhdl.children_name[entity_counter].port.append([tmp2[0].strip(),tmp2[1].strip()])
            ...
            if("of" in tmp2[0]):
                ...
                tmp2[0]=tmp4[1].strip()
            if(len(tmp3)>1):
                entity_vhdl.attribute.append([tmp2[0].strip(),tmp3[0].strip(),tmp3[1].strip(),vhdl_line_str])
            else:
                entity_vhdl.attribute.append([tmp2[0].strip(),tmp3[0].strip(),vhdl_line_str])
